...
class subCountyController {
  static newSubCounty(req, res) {
    ...
    pool.query('SELECT * FROM county WHERE id = $1', [id], (error, results) => {
      if (results.rowCount > 0) {
        pool.query(
          'INSERT INTO sub_county (name, male, female, total, county_id) VALUES($1, $2, $3, $4, $5)',
  ...
  static editSubCounty(req, res) {
    ...
    pool.query('SELECT * FROM sub_county WHERE id = $1', [id], (error, results) => {
      ...
      pool.query(
        'UPDATE sub_county set name = ($1), male=($2), female=($3), total=($4) WHERE id = ($5)',
        [name, male, female, total, id],
        (err) => {
          if (err) {
            return res.status(400).json({
  ...
  static deleteSubCounty(req, res) {
    ...
    pool.query('SELECT * FROM sub_county WHERE id = $1', [id], (error, results) => {
      ...
      pool.query('DELETE FROM sub_county WHERE id = ($1)', [id], (err) => {
        if (err) {
          return res.status(400).json({
            message: 'Error',
            error: err,
  ...
  static allSubCounties(req, res) {
    pool.query('SELECT * FROM sub_county ORDER BY id ASC', (error, results) => {
      if (error) {
        return res.status(400).json({
          status: '400',
          message: error,
